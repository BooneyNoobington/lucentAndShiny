...
library(readr)           # Opening text files
library(shinyWidgets)
source("./dbInterop.R")  # Handle queries and statements.

# Module server function
locationsServer <- function(id) {
    shiny::moduleServer(
        id
      , function(input, output, session) {
            ...
            master.query  <- shiny::reactiveVal(readr::read_file("./SQL/QUERY_SPOTS.SQL"))
            ...
            shiny::observeEvent(
                sites.query()
              , {sites.df(DBI::dbGetQuery(db.conn, sites.query()))}
            )
            ...
            output$sites.tbl <- DT::renderDT({
                ...
                DT::datatable(
                  ...
                  , colnames = unlist(coltrans.cnf$locations$sites_table)
                )
            })
            ...
            shiny::observeEvent(
                master.query()
              , {
                  master.df(DBI::dbGetQuery(db.conn, master.query()))
                }
            )
            ...
            shiny::observeEvent(
                master.stmt()
              , {
                    if(!is.null(master.stmt())) DBI::dbExecute(db.conn, master.stmt())
            ...
            output$master.tbl <- DT::renderDT({
                ...
                DT::datatable(
                  ...
                  , rownames = FALSE  # No running numers.
                    # Only one row should be selectable at a time.
                  , selection = list(mode = "single", target = "row")
                  , colnames = unlist(coltrans.cnf$locations$spots_table)
                )
            })
            ...
            shiny::observe({
                click <- shiny::req(input$master.map_draw_all_features)
                    ...
                    if (new.form$geometry$type == "Point") {
                        ...
                        crs.df <- DBI::dbGetQuery(db.conn, "SELECT * FROM `crs`")
            ...
            shiny::observeEvent(
              ...
              , {
                    glue::glue_sql(
                        .con = db.conn
                      , "
                        INSERT INTO `spot` (`name`,  `description`, `x`, `y`, `id_crs`)
                        VALUES({input$name}, {input$description}, {input$x}, {input$y}, 1)
                        "
                    ) %>%
                        master.stmt()
