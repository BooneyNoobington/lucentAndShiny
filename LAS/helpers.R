# Vectorize systemc configuraiton.
...
GetMaxIdentifier <- function(db.conn, numbering.lst) {
    ...
    # To predict the next collection identifier, one needs to know the recent one.
    ...
    if (length(recent_ident.v) == 0) return(0)

    # Return the selected identifier.
    # Transform it to a vector first.
    return(strsplit(recent_ident.v, "·"))
}
...
GetNextIdentifier <- function(recent_ident.chr, numbering.lst) {
    ...
    if(! length(recent_ident.chr) == n) return -1
